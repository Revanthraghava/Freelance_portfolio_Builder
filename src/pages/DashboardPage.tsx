...
import { Plus, Layout, Edit3, Trash2, ExternalLink, LogOut, Rocket, Clock, Briefcase, Download, AlertCircle, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black tracking-tighter">Studio Dashboard</h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            <h2 className="text-4xl font-black tracking-tighter mb-2 italic">Your Portfolios</h2>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[3rem] p-20 text-center border border-slate-100 shadow-sm">
            ...
            <h3 className="text-3xl font-black mb-4 tracking-tight">No portfolios yet.</h3>
            ...
            <Link to="/editor" className="inline-flex items-center gap-3 bg-slate-900 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-slate-800 transition-all">
              Initialize First Portal <Plus size={20} />
            </Link>
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-8">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h3 className="text-2xl font-black tracking-tight mb-1">{portfolio.content.name || 'Untitled Portfolio'}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Link to={`/editor/${portfolio.id}`} className="flex items-center justify-center gap-2 bg-slate-50 text-slate-900 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-100 transition-all">
                      <Edit3 size={16} /> Edit
                    </Link>
                    <Link 
                      to={`/preview/${portfolio.id}`} 
                      className="flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-indigo-100 transition-all"
                    >
                      <ExternalLink size={16} /> Preview
                    </Link>
                    <button 
                      onClick={() => handleDownload(portfolio)}
                      className="col-span-2 flex items-center justify-center gap-2 bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-800 transition-all"
                    >
                      <Download size={16} /> Download ZIP
                    </button>
                    <button onClick={() => setConfirmDeleteId(portfolio.id)} className="col-span-2 flex items-center justify-center gap-2 text-rose-500 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-rose-50 transition-all">
        ...
        {confirmDeleteId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <h3 className="text-2xl font-black tracking-tight mb-2">Delete Portfolio?</h3>
              <p className="text-slate-500 font-medium mb-8 leading-relaxed">
                This action is permanent and cannot be undone. All your portfolio data will be wiped from our servers.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <button 
                  onClick={() => setConfirmDeleteId(null)}
